...
Create DB (one of the following):
```bash
# psql
...
## API (quick reference)
- `POST /train`  
  body: `{ "model_type": "linear" | "random_forest", "params": { ... } }`
- `POST /predict`  
  body: `{ "experiment_id": <int>, "input": { cylinders, displacement, horsepower, weight, acceleration, model_year, origin } }`
- `GET /experiments`
- `GET /predictions?experiment_id=<id>`
- `GET /health`
