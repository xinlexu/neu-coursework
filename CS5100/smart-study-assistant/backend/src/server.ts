...
const SESSION_SECRET_FALLBACK = 'local-development-session-secret';
const isProduction = process.env.NODE_ENV === 'production';
const configuredCorsOrigin = process.env.CORS_ORIGIN || (isProduction ? false : true);
...
const CONFIG = {
	...
	sessionSecret: process.env.SESSION_SECRET || SESSION_SECRET_FALLBACK,
...
async function run(): Promise<void> {
	...
	if (CONFIG.isProduction && CONFIG.sessionSecret === SESSION_SECRET_FALLBACK) {
	...
	try {
		...
		if (CONFIG.enableDemoUser && CONFIG.demoPassword && !await UserStore.exists('demo')) {
			await UserStore.create({
				userId: crypto.randomUUID(),
				username: 'demo',
				passwordHash: await bcrypt.hash(CONFIG.demoPassword, 10)
			});
			console.log('[Server] Demo user created');
		} else if (CONFIG.enableDemoUser && !CONFIG.demoPassword) {
			console.warn('[Server] ENABLE_DEMO_USER is true, but DEMO_PASSWORD is not set; skipping demo user creation');
		}
