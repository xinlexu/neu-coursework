...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--map-file", type=str, default="data/sanjose.pbf", help="Map (.pbf)"
    )
    parser.add_argument(
        "--landmark-file",
        type=str,
        default="data/sanJose-landmarks.json",
        help="Landmarks (.json)",
    )
    parser.add_argument(
        "--path-file",
    ...
    sanJoseMapName = args.map_file.split("/")[-1].split("_")[0]
    sanJoseCityMap = readMap(args.map_file)
    addLandmarks(sanJoseCityMap, args.landmark_file)
    ...
    plotMap(
        cityMap=sanJoseCityMap,
        path=parsedPath,
        waypointTags=parsedWaypointTags,
        mapName=sanJoseMapName,
    )
